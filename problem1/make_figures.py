...
# 科研论文风格：中文宋体(SimSun)作主字体（其拉丁/数字字形为 Times 风格衬线），数学字体 STIX。
# 注：本 matplotlib 版本跨字体回退对 CJK 不生效（Times+回退 会出豆腐块），故采用单一 SimSun 主字体。
# 图宽按论文版心（整栏 ~7.0 in / 178 mm）设定，字号 8–9 pt，保证按原宽插入时字号正确；不画图内标题（题注在文档中）。
plt.rcParams.update({
    "font.family": ["SimSun"],
    "font.serif": ["SimSun"],
